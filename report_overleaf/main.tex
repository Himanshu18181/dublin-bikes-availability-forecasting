...
\title{Predicting Dublin Bikes Station Availability for Data-Driven Rebalancing Using Open Urban Data}
...
Forecasting shared-bicycle availability has an established literature. Froehlich, Neumann and Oliver produced the earliest influential study on Barcelona's Bicing system, clustering stations by usage rhythm and showing that station behaviour is highly predictable from time of day and neighbourhood \cite{b1}. Kaltenbrunner et al.\ extended this with time-series models that predict station occupancy minutes to hours ahead from openly sampled operator data \cite{b2}, establishing the viability of open-feed forecasting that this project also adopts.

Directly relevant to Dublin, Yoon, Pinelli and Calabrese built Cityride, a journey advisor that predicts station-level availability on the dublinbikes system itself and recommends pick-up and return stations accordingly \cite{b3}. Their work validates the dublinbikes feed as a research-grade data source, but targets the rider; the present project targets the operator.

On the operational side, O'Mahony and Shmoys formulated rebalancing for New York's Citi Bike as clustering and routing optimisation problems whose tooling entered daily production use \cite{b4}, demonstrating the business value of pairing prediction with dispatch planning, a pattern this project reproduces at smaller scale with its greedy nearest-surplus planner. Li et al.\ predict city-wide and cluster-level rental and return flows with gradient-boosted regression trees \cite{b5}, and Hulot, Aloise and Jena show that station-level demand models built on temporal and weather features, evaluated with scikit-learn-family regressors, materially improve rebalancing efficiency \cite{b6}.

Methodologically, the model ladder used here rests on Breiman's random forests \cite{b7} and on gradient-boosted trees as implemented in XGBoost \cite{b8}, with all pipelines expressed in scikit-learn \cite{b9}. The consistent finding across this literature, that calendar structure dominates short-horizon bike availability, motivates both the feature set and the honest baseline used in this paper.

\section{Data and Methodology}
\subsection{Dataset and Preparation}
The data are the official dublinbikes station-status feed published on the Smart Dublin open-data portal by Dublin City Council \cite{b10}. Three monthly files (April, May, June 2026) each contain five-minute snapshots of every station: bikes available, docks available, capacity, coordinates and operational status. Table~\ref{tab:data} summarises the corpus.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/eda_occupancy_heatmap.png}}
...
\subsection{Models}
Four approaches of increasing capacity are compared. The \textbf{naive baseline} predicts the historical mean of the target for the same station, day of week and hour over the training months, falling back to the global mean for unseen combinations; any learned model must beat this lookup table to justify its complexity. \textbf{Linear regression} fits ordinary least squares on the encoded features. The \textbf{random forest} \cite{b7} uses 200 trees with a minimum of three samples per leaf. \textbf{XGBoost} \cite{b8} uses 500 boosted trees of maximum depth 7 with learning rate 0.05 and subsampling of 0.9. All models are wrapped in identical scikit-learn pipelines \cite{b9} so that encoding and scaling are fitted on training data only. Predictions are clipped below at zero, since negative bike counts are meaningless.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/station_profile.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/error_by_hour.png}}
...
Future work follows directly from the observed failure modes. Weather covariates and lagged availability (the state one and two hours earlier) target exactly the rush-shoulder errors where the calendar model is weakest; a probabilistic formulation would let alerts carry calibrated confidence; and coupling the planner's move list to a vehicle-routing optimiser in the style of O'Mahony and Shmoys \cite{b4} would close the loop from prediction to dispatch.

% ------------------------------------------------------------------
% PASTE THE REFERENCES BLOCK (\begin{thebibliography} ... \end{thebibliography})
% HERE BEFORE \end{document}. The block is provided separately.
% ------------------------------------------------------------------
